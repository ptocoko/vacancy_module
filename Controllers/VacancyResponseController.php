	...
	public function postResponse()
	{
		if (!empty($_SESSION['login'])) {
			...
			if ($this->respondedAlready($userId, $vacanciId)) {
				return $this->BadRequest(400, 'Вы уже отправили отклик на данную вакансию');
			}
			$response_date = time();
			...
			if ($userId && $vacanciId) {
				...
				try {
					return json_encode($this->repository->saveResponse($vacanciId, $userId, $response_date, $response_comment, $rday));
				} catch (Exception $e) {
					return $this->BadRequest();
				}
			} else {
			...
			return $this->BadRequest(401, 'Пожалуйста зайдите в свой личный кабинет или зарегестрируйтесь');
		}
	}
