...
class VacancyController extends AbstractController
{
	...
	public function getBySorting()
	{
		...
		return json_encode($this->repository->findBySorting($areaCode, $schoolId, $doljnostId, $payment, $stajId,
			$offset * 9));
	...
	public function postVacancy()
	{
		try {
			if (!$director
				= json_decode($this->participDirectorController->getById(),
				true, 512)
			) {
				return $this->BadRequest(401,
		...
		if ($doljnostid != 0 && $stajId != 0) {
			try {
				SimpleRouter::response()->header('Content-Type: application/json');
				$vacancy = $this->repository->saveVacancy($doljnostid, $zp,
					$stajId, $dopInfo, $director['id'], $director['schoolid'],
					$school['AreaCode'], $dateInsert);
				$vacancy['resp'] = [];
