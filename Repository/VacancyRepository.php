...
class VacancyRepository extends AbstractRepository
{
    ...
    public function findBySorting($areaCode, $schoolId, $doljnostId, $payment, $stajId, $offset = 0): array
    {
        $sql = sprintf(
            ...
            $this::getTableName(),
            DoljnostRepository::getTableName(),
    ...
    public function findBySchoolId(string $schoolId): array
    {
        $stmt = $this->dbo->prepare(
            "SELECT  v.id, v.zp, v.date_insert, v.dop_info, v.schoolid, v.id_directora, v.doljnost_id, v.staj_id, d.name, d.typedoljnostid as dtype, s.value
    ...
    public function countOfVacanciesWith($doljnostId, $schoolId)
    {
        $stmt = $this->dbo->query(
            ...
            ) . " WHERE doljnost_id = " . $doljnostId . " AND schoolid = " . $schoolId
        );
